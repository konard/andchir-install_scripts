...
#===============================================================================
#
#   Rocket.Chat + MongoDB - Automated Installation Script
#   For Ubuntu 24.04 LTS
#
#   This script automatically installs and configures:
#   - Docker and Docker Compose
#   - MongoDB 6.0 database with replica set (via Docker)
#   - Rocket.Chat messaging platform (via Docker)
...
# MongoDB will use replica set without authentication (as per official setup)
# This is the recommended approach for Rocket.Chat
...
show_usage() {
    ...
    echo ""
    echo "Arguments:"
    echo "  domain_name    The domain name for Rocket.Chat (e.g., chat.example.com)"
    echo ""
    echo "Example:"
...
show_banner() {
    ...
    echo -e "${CYAN}   ║${NC}   ${BOLD}${WHITE}Rocket.Chat + MongoDB${NC}                                                    ${CYAN}║${NC}"
    ...
    echo -e "${CYAN}   ║${NC}   ${GREEN}•${NC} MongoDB ${MONGODB_VERSION} database with replica set (via Docker)                  ${CYAN}║${NC}"
    echo -e "${CYAN}   ║${NC}   ${GREEN}•${NC} Rocket.Chat messaging platform                                        ${CYAN}║${NC}"
...
create_installation_directory() {
    ...
    su - "$CURRENT_USER" -c "mkdir -p '$INSTALL_DIR/data/mongodb'"
...
create_env_file() {
    ...
    # Check if .env file already exists with configuration
    if [[ -f "$INSTALL_DIR/.env" ]]; then
        print_info "Environment file already exists at $INSTALL_DIR/.env"

        # Update domain if changed
        print_step "Updating domain configuration..."
        sed -i "s|^DOMAIN=.*|DOMAIN=$DOMAIN_NAME|" "$INSTALL_DIR/.env"
        sed -i "s|^ROOT_URL=.*|ROOT_URL=https://$DOMAIN_NAME|" "$INSTALL_DIR/.env"
...
services:
  mongodb:
    ...
    environment:
      ...
      MONGODB_INITIAL_PRIMARY_PORT_NUMBER: ${MONGODB_PORT_NUMBER:-27017}
...
start_docker_containers() {
    ...
    print_step "Waiting for MongoDB replica set to initialize..."
    sleep 15

    # Wait for Rocket.Chat to be ready
    print_step "Waiting for Rocket.Chat to be ready (this may take a few minutes)..."
    local retries=60
    local count=0
    while [[ $count -lt $retries ]]; do
        if curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1:$APP_PORT/api/info" 2>/dev/null | grep -q "200"; then
            print_success "Rocket.Chat is ready and responding"
            break
        fi
        ((count++))
        sleep 5
    ...
    if [[ $count -eq $retries ]]; then
        print_warning "Rocket.Chat may not be fully ready yet. Check logs with: docker compose logs -f rocketchat"
    fi
}
...
case "$1" in
    ...
    restart)
        ...
        ;;
    logs)
        docker compose logs -f "${2:-rocketchat}"
        ;;
    status)
        docker compose ps
        ;;
    ...
    backup)
        ...
        BACKUP_NAME="rocketchat-backup-$(date +%Y%m%d-%H%M%S)"
        mkdir -p "$BACKUP_DIR"
        echo "Creating backup: $BACKUP_NAME"
        docker compose exec -T mongodb mongodump --archive --gzip > "$BACKUP_DIR/$BACKUP_NAME.gz"
        echo "Backup saved to: $BACKUP_DIR/$BACKUP_NAME.gz"
        ;;
    restore)
        if [[ -z "$2" ]]; then
        ...
        docker compose exec -T mongodb mongorestore --archive --gzip < "$2"
        echo "Restore completed"
        ;;
    *)
        echo "Rocket.Chat Management Script"
        echo ""
        echo "Usage: $0 {start|stop|restart|logs|status|update|backup|restore}"
        echo ""
        echo "Commands:"
        echo "  start         - Start Rocket.Chat containers"
        echo "  stop          - Stop Rocket.Chat containers"
        echo "  restart       - Restart Rocket.Chat containers"
        echo "  logs          - Show logs (optional: logs mongodb)"
        echo "  status        - Show container status"
        echo "  update        - Update Rocket.Chat to latest version"
...
show_completion_message() {
    ...
    echo -e "${WHITE}MongoDB Database:${NC}"
    echo -e "  ${CYAN}•${NC} Host:             ${BOLD}mongodb (Docker internal)${NC}"
    echo -e "  ${CYAN}•${NC} Port:             ${BOLD}27017${NC}"
    echo -e "  ${CYAN}•${NC} Database:         ${BOLD}rocketchat${NC}"
    echo -e "  ${CYAN}•${NC} Replica Set:      ${BOLD}rs0${NC}"
    echo ""

    echo -e "${WHITE}Rocket.Chat Configuration:${NC}"
    ...
    echo -e "  ${CYAN}•${NC} Restore:          ${BOLD}$INSTALL_DIR/manage.sh restore <file>${NC}"
    echo ""

    echo -e "${YELLOW}Important:${NC}"
    echo -e "  ${CYAN}•${NC} Configuration is stored in: ${BOLD}$INSTALL_DIR/.env${NC}"
    echo -e "  ${CYAN}•${NC} On first access, you will need to create an admin account"
    echo ""

    echo -e "${YELLOW}Next Steps:${NC}"
    echo -e "  ${CYAN}1.${NC} Visit ${BOLD}https://$DOMAIN_NAME${NC} to access Rocket.Chat"
    echo -e "  ${CYAN}2.${NC} Complete the setup wizard to create your admin account"
    echo -e "  ${CYAN}3.${NC} Configure organization name and settings"
    echo -e "  ${CYAN}4.${NC} Check ${BOLD}https://docs.rocket.chat${NC} for documentation"
    echo ""

    print_success "Thank you for using Rocket.Chat + MongoDB installer!"
    echo ""
}
