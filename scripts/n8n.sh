...
#===============================================================================
#
#   n8n + PostgreSQL - Automated Installation Script
#   For Ubuntu 24.04 LTS
#
#   This script automatically installs and configures:
#   - Docker and Docker Compose
#   - PostgreSQL 16 database (via Docker)
#   - n8n workflow automation platform (via Docker)
...
show_usage() {
    ...
    echo ""
    echo "Arguments:"
    echo "  domain_name    The domain name for n8n (e.g., n8n.example.com)"
    echo ""
    echo "Example:"
    echo "  $0 n8n.example.com"
    echo ""
...
validate_domain() {
    ...
    if [[ ! "$domain" =~ ^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?)*\.[a-zA-Z]{2,}$ ]]; then
        ...
        print_info "Please enter a valid domain (e.g., n8n.example.com)"
        exit 1
    fi
}
...
generate_encryption_key() {
    # Generate a secure encryption key for n8n (32 bytes hex)
    openssl rand -hex 32
}
...
show_banner() {
    ...
    echo -e "${CYAN}   ║${NC}   ${BOLD}${WHITE}n8n + PostgreSQL${NC}                                                        ${CYAN}║${NC}"
    ...
    echo -e "${CYAN}   ║${NC}   ${GREEN}•${NC} n8n workflow automation platform                                      ${CYAN}║${NC}"
...
create_installation_directory() {
    ...
    su - "$CURRENT_USER" -c "mkdir -p '$INSTALL_DIR/data/postgres'"
    su - "$CURRENT_USER" -c "mkdir -p '$INSTALL_DIR/data/n8n'"
...
create_env_file() {
    ...
    if [[ -f "$INSTALL_DIR/.env" ]]; then
        ...
        DB_ROOT_PASSWORD=$(grep "^POSTGRES_ROOT_PASSWORD=" "$INSTALL_DIR/.env" 2>/dev/null | cut -d'=' -f2 || echo "")
        N8N_ENCRYPTION_KEY=$(grep "^N8N_ENCRYPTION_KEY=" "$INSTALL_DIR/.env" 2>/dev/null | cut -d'=' -f2 || echo "")
        ...
        sed -i "s|^N8N_HOST=.*|N8N_HOST=$DOMAIN_NAME|" "$INSTALL_DIR/.env"
        sed -i "s|^WEBHOOK_URL=.*|WEBHOOK_URL=https://$DOMAIN_NAME/|" "$INSTALL_DIR/.env"
...
start_docker_containers() {
    ...
    while [[ $count -lt $retries ]]; do
        if curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1:$APP_PORT/healthz" 2>/dev/null | grep -q "200"; then
            print_success "n8n is ready and responding"
            break
        fi
        ((count++))
        sleep 2
    ...
    if [[ $count -eq $retries ]]; then
        print_warning "n8n may not be fully ready yet. Check logs with: docker compose logs -f n8n"
    fi
}
...
show_completion_message() {
    ...
    echo -e "  ${CYAN}•${NC} Host:             ${BOLD}postgres (Docker internal)${NC}"
    ...
    echo -e "${WHITE}n8n Configuration:${NC}"
    ...
    echo -e "  ${CYAN}•${NC} On first access, you will need to create an owner account"
    echo ""

    echo -e "${YELLOW}Next Steps:${NC}"
    echo -e "  ${CYAN}1.${NC} Visit ${BOLD}https://$DOMAIN_NAME${NC} to access n8n"
    echo -e "  ${CYAN}2.${NC} Create your owner account on first login"
    echo -e "  ${CYAN}3.${NC} Start building your automation workflows"
    echo -e "  ${CYAN}4.${NC} Check ${BOLD}https://docs.n8n.io${NC} for documentation"
    echo ""

    print_success "Thank you for using n8n + PostgreSQL installer!"
    echo ""
}
